...
The posts created from Instagram photos can be saved as draft or directly published. There's also the possibility of programming automatic creation of posts from Instagram photos.
...
= Automatic creation of posts from Instagram photos doesn't seem to work. What's wrong? =
Due to how event scheduling works in WordPress, scheduled events can only start on page visit. This is how it works: on a page visit, the system checks if there's any scheduled events and, for each event, checks if enough time has passed since the last execution. If enough time has passed, it executes the scheduled event. So, non automatic posting can happen without visiting pages.

= Automatic creation of posts from Instagram photos only work with user stream photos, while the tag stream photos are ignored. What's wrong? =
